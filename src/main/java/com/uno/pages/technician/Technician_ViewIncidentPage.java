...
public class Technician_ViewIncidentPage extends BaseCode{
	...
	public void displayAsPerArrowsOfPagination() throws Exception {
		...
		highLight(arrow_LeftmostArrowOfPagination);
		...
		// Thread.sleep(500);
		highLight(arrow_RightmostArrowOfPagination);
		driver.manage().timeouts().pageLoadTimeout(150, TimeUnit.SECONDS);
		...
		highLight(arrow_RightArrowOfPagination);
		driver.manage().timeouts().pageLoadTimeout(150, TimeUnit.SECONDS);
		...
		highLight(arrow_LeftArrowOfPagination);
		driver.manage().timeouts().pageLoadTimeout(150, TimeUnit.SECONDS);
	...
	// column headers should display as per selected in My View pop-up of View
		...
		public void displayDescendingOrderOfIncidentID() throws Exception {
			...
			for (int i = 0; i < countOfRow - 1; i++) {
				...
				// Get the First ID
				String textOfFirstID = lstIDInTable.get(i).getText();
				// Convert into integer
				int firstID = Integer.parseInt(textOfFirstID);
				System.out.println("firstID: "+firstID);
				// Get the Second ID
				highLight(lstIDInTable.get(i+1));
				String textOfSecondID = lstIDInTable.get(i + 1).getText();
				// Convert into integer
